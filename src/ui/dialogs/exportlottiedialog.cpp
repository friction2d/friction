                                          ...
                                          const QString& htmlFile)
{
    ...
    stream << "const update=()=>{const t=total();const c=current();scrub.max=String(t-1);if(!dragging){scrub.value=String(c);}frame.textContent=(c+1)+' / '+t;};\n";
    ...
    stream << "const onComplete=()=>{if(mode.value==='pingpong'){direction*=-1;anim.setDirection(direction);anim.goToAndPlay(direction>0?first():first()+total()-1,true);}else if(mode.value==='once'){onceCompleted=true;}update();};\n";
    ...
    stream << "const togglePlayback=()=>{if(!anim){return;}if(anim.isPaused&&mode.value==='once'&&onceCompleted){onceCompleted=false;direction=1;anim.setDirection(1);anim.goToAndPlay(first(),true);}else if(anim.isPaused){anim.play();}else{anim.pause();}update();};\n";
    ...
    stream << "renderer.addEventListener('click',()=>{const rel=current();const paused=!anim||anim.isPaused;cycle(renderer,rendererOptions);createAnimation(first()+rel,!paused);applyWireframe();});\n";
    stream << "speed.addEventListener('click',()=>{cycle(speed,speedOptions);anim.setSpeed(parseFloat(speed.value)||1);});\n";
    stream << "scrub.addEventListener('input',()=>{dragging=true;onceCompleted=false;anim.goToAndStop(first()+(parseInt(scrub.value,10)||0),true);update();});\n";
